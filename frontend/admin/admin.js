...
//     async function loadUserManagementData() - Load admin users data (DONE BY PRETI)
//     function updateUserManagementTable() - Update users management table (DONE BY PRETI)
...
//     async function performUserDeletion() - Execute user deletion (DONE BY PRETI)
//
...
//     window.addEventListener('DOMContentLoaded') - Check login status on page load
//
...
function filterFeedbackData() {
    ...
    filteredFeedbackData = allFeedbackData.filter(item => {
        ...
        // Email filter - CRITICAL: Proper detection
        if (emailFilter) {
...
function renderFeedbackPage() {
    ...
    pageData.forEach(feedback => {
        ...
        // Actions
        row.insertCell(9).innerHTML = `<button class="btn-delete" onclick="deleteFeedback(${feedback.id})">🗑️ Delete</button>`;
    });
...
function renderArchivePage() {
    ...
    pageData.forEach(feedback => {
        ...
        // Actions
        row.insertCell(10).innerHTML = `<button class="btn-delete" onclick="deleteArchiveFeedback(${feedback.id})">🗑️ Delete</button>`;
    });
...
function updateUserManagementTable(usersData) {
    ...
    usersData.forEach(user => {
        ...
        const isProtectedRootUser = safeUser.username === 'systemadmin'; // Protected root account
...
async function downloadExport(exportType) {
    try {
        const username = sessionStorage.getItem('loggedUser'); // ✅ FIXED: Use correct key
        const userRole = sessionStorage.getItem('userRole');
        ...
        const response = await fetch(`/api/admin/data-export/${exportType}`, {
            method: 'GET',
            credentials: 'same-origin' // ✅ INCLUDE SESSION COOKIE - all auth via session now
        });
...
function showPage(pageName) {
    ...
    // Note: data-export page doesn't need data loading, just initialization
}
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    if (loggedUser) {
        ...
        // ✅ CRITICAL: Load user's theme settings on page load
...
const themeConfig = {
    sections: [
        { id: 'saved-themes', name: 'Saved Themes', icon: '💾', isSavedThemes: true },  // ← ADD THIS LINE FIRST
    ...
    // UPDATED selector mapping with corrected groupings
    selectorMapping: {
        ...
        'theme-settings-page': {
            ...
            card: {
                ...
                'card-secondary-text': [
                    '#theme-settings-page .preview-card p',
                    '#theme-settings-page .theme-section-item',       // ✅ Added 's'
                    '#theme-settings-form .theme-group-title'          // ✅ ADD THIS LINE
                ],
            },
            buttons: {
                'button-color': [
    ...
    // UPDATED color options to match new groupings
    colorOptions: {
        general: [
...
function renderThemeControls(sectionId) {
    ...
    // ✅ FOR OTHER SECTIONS: Hide saved-themes panel, show color controls
    if (savedThemesPanel) {
        ...
        <div class="theme-control-input">
            <div class="color-picker-wrapper">
                <input type="color" 
                    ...
                    value="${colorPickerValue}"          ✅ FIXED: Uses 6-digit hex only
...
// ============================================================
// SAVED THEMES JAVASCRIPT - ADD TO admin.js
// ============================================================
// Add these functions to your admin.js file
// Place them in the "THEME SETTINGS MANAGEMENT" section
 ...
 * ✅ NOW HANDLES: Both old format and new format with useGlobal
 */
 ...
 * ✅ NOW CAPTURES: ONLY Admin Overall (global) colors
 */
...
// The "Load" button has been removed in favor of a simpler "Set Active" only approach
// Users now only use "Set Active" which applies the theme AND persists it


/**
 * Apply theme data to color inputs
 * ✅ NOW APPLIES: ONLY Admin Overall (global) colors
...
function applyThemeData(themeData) {
    try {
        ...
        // ✅ CRITICAL: Save to user-specific localStorage so it persists!
        const currentUserId = sessionStorage.getItem('userId') || 
                             sessionStorage.getItem('loggedUser');
        if (currentUserId) {
            const storageKey = `adminThemeSettings_user_${currentUserId}`;
        ...
        // ✅ CRITICAL: Apply the theme to the actual page elements
...
async function loadActiveThemeOnLogin() {
    try {
        ...
        if (data.theme) {
            ...
            // ✅ CRITICAL: Load individual page settings after theme
            loadIndividualPageSettings();
...
function showConfirmDialog(title, message, confirmText = 'Confirm', type = 'primary') {
    return new Promise((resolve) => {
        // You can implement your own custom modal here
