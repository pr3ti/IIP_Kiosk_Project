...
//   - selectedRetention, selectedTheme, userData, stream, etc.
//   - INACTIVITY_TIMEOUT (5 minutes)
//
// INITIALIZATION & SETUP
//   - loadDynamicQRCode()          - Load dynamic QR code from server
//   - detectDeviceType()           - Detect mobile/desktop device
//   - DOMContentLoaded handlers    - Initial page setup
//
// INACTIVITY TIMER MANAGEMENT
//   - startInactivityTimer()       - Start 5-minute countdown
//   - resetInactivityTimer()       - Reset on user interaction
//   - returnToLandingPage()        - Return to start when timeout
//   - showTimeoutNotification()    - Show timeout message
//
// QUESTION MANAGEMENT
//   - loadFeedbackQuestions()      - Load questions from database
//   - updateFeedbackForm()         - Update form with questions
//   - createQuestionElement()      - Create question UI elements
//   - selectQuestionRating()       - Handle star rating selection
//   - initializeQuestionEventListeners() - Setup question events
//   - showNoQuestionsMessage()     - Show message if no questions
//   - getQuestionType()            - Determine question type
//   - validateRequiredQuestions()  - Validate required answers
//
// FORM SUBMISSION
//   - submitFeedback()             - Submit feedback form
//   - submitDetails()              - Submit user details
//   - submitPledge()               - Submit pledge and redirect
//
// PHOTO HANDLING
//   - handlePhotoUpload()          - Handle file upload (mobile)
//   - continueToStyleFromUpload()  - Continue from upload to style
//   - retakePhotoFromUpload()      - Retake photo from upload page
//   - initializeCamera()           - Initialize camera (desktop only)
//   - capturePhoto()               - Capture with countdown/redirect
//   - takePhoto()                  - Take photo from camera stream
//   - continueToStyle()            - Go to style page after photo
//   - saveOriginalPhoto()          - Save original to server
//   - saveProcessedPhoto()         - Save processed photo to server
//
// OVERLAY & THEME MANAGEMENT
//   - loadOverlayOptions()         - Load overlays from database
//   - loadDefaultOverlayOptions()  - Fallback default overlays
//   - generateColorFromThemeId()   - Generate consistent colors
//   - selectTheme()                - Select theme and update preview
//   - updateThemePreview()         - Update theme preview image
//   - updatePreviewWithCutout()    - Update preview with positioning
//   - processFinalPhoto()          - Process final photo with overlay
//
// PAGE NAVIGATION
//   - showConsentPage()            - Show consent page
//   - selectOption()               - Select retention option
//   - showDetailsPage()            - Show details page
//   - retakePhotoFromStyle()       - Retake photo from style page
//   - confirmStyle()               - Confirm and go to confirmation
//   - updateConfirmationDetails()  - Update confirmation page
//   - goBackToStyle()              - Go back to style page
//   - finalSubmit()                - Final submission with saving
//   - submitAnother()              - Reset and start new submission
//
// BACK NAVIGATION FUNCTIONS
//   - goBackToLanding()            - Consent → Landing
//   - goBackToConsent()            - Details → Consent
//   - goBackToDetails()            - Feedback → Details
//   - goBackToFeedback()           - Pledge → Feedback
//   - goBackToPledge()             - Photo/Upload → Pledge
//   - goBackToPhoto()              - Style → Photo/Upload
//
// EVENT LISTENERS & CLEANUP
//   - beforeunload cleanup         - Clean up camera and timers
//   - User interaction listeners   - Reset inactivity timer
//
// ============================================================
...
function returnToLandingPage() {
    ...
    const allPages = document.querySelectorAll('.container > div');
...
function updateFeedbackForm(questions) {
    ...
    questions.forEach((question, index) => {
        if (!question.is_active) return; // Skip inactive questions
...
function getQuestionType(questionElement) {
    ...
    if (questionElement.querySelector('.question-rating')) return 'stars';
...
function submitFeedback(event) {
    ...
    allQuestions.forEach(question => {
        ...
        userData[`q${questionId}`] = answerValue; // Keep for backward compatibility
    });
...
function confirmStyle() {
    // Process the final photo with overlay (but don't save yet)
...
function finalSubmit() {
    ...
    .then(data => {
        ...
        // Don't restart timer on thank you page - let user read the message
    })
    .catch(error => {
