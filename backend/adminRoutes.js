...
// 
// 7. PHOTO ACCESS & EMAIL DECRYPTION ROUTES
...
//    router.get('/users'              - Get all admin users with role information (DONE BY PRETI)
//    router.delete('/users/:id'       - Delete admin user with soft delete (DONE BY PRETI)
//    router.post('/users'             - Add new admin user with password hashing (DONE BY PRETI)
//    router.put('/users/:id'          - Update admin user details with validation (DONE BY PRETI)
//
// 9. DATA EXPORT MANAGEMENT ROUTES
//    router.post('/data-export/unlock' - Unlock data export with password verification (System Admin only) (DONE BY PRETI)
//
// 10. EXPORT/IMPORT ROUTES
...
// Dashboard data endpoint - UPDATED to show only last 1 month of data
...
// ==================== ARCHIVE DELETION ROUTES (System Admin Only) ====================
...
// Get all admin users - UPDATED to include full_name
router.get('/users', (req, res) => {
    ...
    db.get(tableCheckQuery, [], (err, table) => {
        ...
        // ✅ UPDATED: Include full_name in query
        const query = `
            SELECT 
                id,
                username,
        ...
        db.all(query, [], (err, rows) => {
            ...
            const users = rows.map(user => ({
                ...
                full_name: user.full_name || user.username || 'Unknown', // ✅ Use full_name if available
...
// Add new admin user - UPDATED to save full_name
router.post('/users', async (req, res) => {
    ...
    db.get(checkUserQuery, [currentUsername], async (err, currentUser) => {
        ...
        // ✅ FIXED: Allow all three roles including system_admin
        ...
        db.get(checkUsernameQuery, [username], async (err, existingUser) => {
            ...
            try {
                ...
                // ✅ FIXED: Insert new user WITH full_name
                const insertQuery = `
                    INSERT INTO admin_users (username, full_name, password_hash, role, created_at, is_active)
                    ...
                    db.get(getUserQuery, [this.lastID], (err, newUser) => {
                        ...
                        res.json({
                            ...
                            user: {
                                ...
                                full_name: newUser.full_name, // ✅ Return full_name
                                role: newUser.role,
...
// Delete admin user - UPDATED for HARD DELETE
...
// Update admin user - UPDATED to handle full_name
    ...
    db.get(checkUserQuery, [currentUsername], async (err, currentUser) => {
        ...
        db.get(getTargetUserQuery, [id], async (err, targetUser) => {
            ...
            async function performUserUpdate() {
                ...
                if (role !== undefined) {
                    // Allow role change for all users (including system_admin)
                ...
                if (password && password.trim() !== '') {
                    try {
                        // Hash the new password using async/await
                        const hashedPassword = await auth.hashPassword(password);
...
router.post('/data-export/unlock', (req, res) => {
    ...
    db.get(getUserQuery, [username], async (err, dbUser) => {
        ...
        try {
            ...
            // CRITICAL FIX: Set session flags for data export unlock
            req.session.data_export_unlocked = true;
            req.session.data_export_unlock_time = new Date().toISOString();
            
            // CRITICAL FIX: Explicitly save session before responding
...
// Excel download endpoint - UPDATED with email decryption
router.get('/download-excel', (req, res) => {
    ...
    auth.loginUser(username, password, (err, user) => {
        ...
        db.all(query, [], (err, rows) => {
            ...
            // *** NEW: Decrypt emails before converting to CSV ***
...
// Archive Excel download endpoint - NEW with email decryption
...
router.get('/questions', (req, res) => {
    ...
    db.get(tableCheckQuery, [], (err, table) => {
        ...
        db.all(questionsQuery, [], (err, questions) => {
            ...
            // ✅ Fix: Use Map to preserve order during async processing
                ...
                if (question.question_type === 'choice') {
                    ...
                    db.all(optionsQuery, [question.id], (err, options) => {
                        if (err) {
                            ...
                            // ✅ Update in place - preserves original order
...
// Add new question - COMPLETED AND FIXED
router.post('/questions', (req, res) => {
    ...
    db.get(checkUserQuery, [username], (err, user) => {
        ...
        // Validate question type - UPDATED TO MATCH admin.js
                ...
                if (question_type === 'choice' && options && options.length > 0) {
                    ...
                    options.forEach((option, index) => {
                        const insertOptionQuery = `
                            INSERT INTO question_options (question_id, option_label, display_order, created_at)
                            VALUES (?, ?, ?, CURRENT_TIMESTAMP)
                        `;
