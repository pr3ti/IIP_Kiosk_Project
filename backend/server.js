...
//
// 5. API ROUTES
//    app.use('/api/feedback', feedbackRoutes) - Feedback API routes
...
//    function generateSelfSignedCertificate() - Generate self-signed certificate
...
const dataExportRoutes = require('./dataExportRoutes'); // ✨ NEW: Data export routes
const os = require('os');
...
app.use(session({
    secret: process.env.SESSION_SECRET || 'your-secret-key-change-in-production',
    resave: true,  // ← Change from false to true
    saveUninitialized: false,
...
app.use('/api/admin/data-export', dataExportRoutes); // ✨ NEW: Data export routes (system admin only)
...
function printServerInfo(isHttps) {
    ...
    console.log(`📦 Data Export: ${isHttps ? 'https' : 'http'}://${localIP}:${PORT}/api/admin/data-export`); // ✨ NEW
